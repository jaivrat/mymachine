...
public class Percolation {
   ...
   public void open(int i, int j)         // open site (row i, column j) if it is not already
   {
      ...
      //explore X+ (row, col+1) 
      if ((col+1 < mN) && multi[row][col+1] == true)
      {
       ...
       //explore Y+ (row+1,col) 
       if ((row+1 < mN) && multi[row+1][col] == true) //check i+1,j
       {
        ...
        //explore Y-
       if (row > 0 && multi[row-1][col] == true)    //check i-1,j
       {
           cont.union(idx - mN, idx);
           backtrack.union(idx - mN, idx);
       }
...
//   }
   ...
   {
       checkRange(i, j);
       return (multi[i-1][j-1] == true && backtrack.connected(mN*(i-1)+j-1, mTopSentIdx));
